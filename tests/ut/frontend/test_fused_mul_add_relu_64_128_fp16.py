...
correct pto.alloc_tile / pto.tload / pto.tmul / pto.tadd / pto.trelu patterns.
"""
...
# Kernel: load three tiles, multiply lhs with out, add rhs, then apply ReLU
@fe.kernel
def fused_mul_add_relu_kernel(
    ...
    with pl.section_vector():
        ...
        plm.load(tile_c, z, [0, 0])
        pl.system.sync_src(set_pipe=pl.PipeType.MTE2, wait_pipe=pl.PipeType.V, event_id=0)
        pl.system.sync_dst(set_pipe=pl.PipeType.MTE2, wait_pipe=pl.PipeType.V, event_id=0)
        plm.fused_mul_add_relu(tile_c, tile_a, tile_b)
...
def test_fused_mul_add_relu():
    ...
    z = torch.rand(shape, device=device, dtype=dtype)  # z has initial value
    z_orig = z.clone()

    compiled_lib = fe.compile(fused_mul_add_relu_kernel, arch="a3")
