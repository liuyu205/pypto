...
correct pto.alloc_tile / pto.tload / pto.tsub / pto.tmul patterns.
"""
...
# Kernel: load two tiles, subtract them, apply relu, and cast result
@fe.kernel
def sub_relu_cast_kernel(
    ...
    with pl.section_vector():
        ...
        plm.sub_relu_cast(tile_c, tile_a, tile_b, target_type=pl.FP32, mode="round")
...
def test_sub_relu_cast():
    ...
    compiled_lib = fe.compile(sub_relu_cast_kernel, arch="dav-c220-vec")
    ...
    print(z)
    z_sub = x - y
    z_relu = torch.relu(z_sub).to(dtype_out)
    print("***********golden output***********")
    print(z_relu.shape, z_relu.dtype)
    print(z_relu)
    torch.testing.assert_close(z, z_relu)
    print("result equal!")
