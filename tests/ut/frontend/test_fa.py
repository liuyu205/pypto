...
"""
 1. 右矩阵转置的两种写法
  ...
  ┌──────────────┬────────────────────────────────────────────┬───────────────────────────────────────────────────┐
  │              │            写法 1: ND + MAT中转             │              写法 2: DN + RIGHT直载               │
  ├──────────────┼────────────────────────────────────────────┼───────────────────────────────────────────────────┤
  │ GlobalTensor │ shape=[K,N], stride=[-1,1] → runtime [N,1] │ shape=[K,N], stride=[-1,1] → 但被 DN 解释为 [1,K] │
  ├──────────────┼────────────────────────────────────────────┼───────────────────────────────────────────────────┤
  │ Layout       │ ND (行主序)                                │ DN (列主序)                                       │
  ├──────────────┼────────────────────────────────────────────┼───────────────────────────────────────────────────┤
  │ Tile shape   │ [K_tile, N_tile] (如 [64,128])             │ [K_tile, N_tile] (如 [64,128])                    │
  ├──────────────┼────────────────────────────────────────────┼───────────────────────────────────────────────────┤
  │ blayout      │ ColMajor (2) — NZ fractal 格式             │ RowMajor (1)                                      │
  ├──────────────┼────────────────────────────────────────────┼───────────────────────────────────────────────────┤
  │ slayout      │ RowMajor (1)                               │ ColMajor (2)                                      │
  ├──────────────┼────────────────────────────────────────────┼───────────────────────────────────────────────────┤
  │ 转置机制     │ 行主序读 + NZ fractal 排列实现转置         │ 列主序读（硬件DMA按列取数据）实现转置             │
  └──────────────┴────────────────────────────────────────────┴───────────────────────────────────────────────────┘

  写法 1（当前代码走的路径）：
  - GlobalTensor 是标准行主序 Layout::ND，stride=[N, 1]
  - k_mat 在 MAT 上分配，tile layout 为 blayout=col_major, slayout=row_major（NZ 格式）
  - TLOAD 按行主序读取 [K_tile, N_tile] 块进 MAT，NZ fractal 自动把数据排列成 Cube 所需的转置格式
  - 然后 TMOV 到 RIGHT（blayout=row_major, slayout=col_major）
  ...
  两者的核心区别：写法 1 的转置在 NZ fractal 数据重排中完成，写法 2 的转置在 DMA 列主序寻址中完成。效果相同，但写法 2 在概念上更清晰（显式声明"我要列主序读取"）。

  3. 当前状态

  当前 layout="nd" 精度正确是因为走的是写法 1：partition_view 从 K 的 ND tensor_view [Skv,D] 中取 [64,128] 大小的块（虽然超出了 D=64 的列边界，但 stride=[D,1] 保证内存访问模式正确——实际上是连续读取
  64×128 个 FP16 元素），然后 MAT tile 的 NZ fractal layout 完成数据重排。
...
def fa_k_kernel_nd_load(
    ...
    with pl.section_cube():
        ...
        # MAT tiles
        q_mat = plm.make_tile(plm.TileType(shape=[TS, TD], dtype=pl.FP16, target_memory=pl.MemorySpace.Mat), addr=MA0, size=Q_F16)
        k_mat = plm.make_tile(plm.TileType(shape=[TD, TKV], dtype=pl.FP16, target_memory=pl.MemorySpace.Mat), addr=MA1, size=KT_F16)
        p_mat = plm.make_tile(plm.TileType(shape=[TS, TKV], dtype=pl.FP16, target_memory=pl.MemorySpace.Mat), addr=MA2, size=P_F16)
        v_mat = plm.make_tile(plm.TileType(shape=[TKV, TD], dtype=pl.FP16, target_memory=pl.MemorySpace.Mat), addr=MA3, size=V_F16)
        # LEFT / RIGHT
        q_left = plm.make_tile(plm.TileType(shape=[TS, TD], dtype=pl.FP16, target_memory=pl.MemorySpace.Left), addr=LA0, size=Q_F16)
        ...
        plm.load(k_mat, k, [0, 0], layout="nd")
        pl.system.sync_src(set_pipe=pl.PipeType.MTE2, wait_pipe=pl.PipeType.MTE1, event_id=1)
        pl.system.sync_dst(set_pipe=pl.PipeType.MTE2, wait_pipe=pl.PipeType.MTE1, event_id=1)
        plm.move(k_right, k_mat)
        pl.system.sync_src(set_pipe=pl.PipeType.MTE1, wait_pipe=pl.PipeType.M, event_id=0)
        ...
        for j in pl.range(1, skv_tiles):
            skv_off = j * TKV
            plm.load(k_mat, k, [skv_off, 0], layout="nd")
            pl.system.sync_src(set_pipe=pl.PipeType.MTE2, wait_pipe=pl.PipeType.MTE1, event_id=1)
            pl.system.sync_dst(set_pipe=pl.PipeType.MTE2, wait_pipe=pl.PipeType.MTE1, event_id=1)
            plm.move(k_right, k_mat)
            pl.system.sync_src(set_pipe=pl.PipeType.MTE1, wait_pipe=pl.PipeType.M, event_id=0)
...
def test_fa_k():
    compiled = fe.compile(fa_k_kernel_nd_load, arch="a3")
